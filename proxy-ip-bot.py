...
MESSAGE_ENTITY_LIMIT = 45 # Maximum number of code blocks per message to avoid Telegram limits
...
def main() -> None:
    ...
    dispatcher = Application.builder().token(BOT_TOKEN).post_init(post_init).build()

    dispatcher.add_handler(addchat_handler)
    dispatcher.add_handler(deletechat_handler)
    dispatcher.add_handler(post_handler)
    dispatcher.add_handler(main_conv_handler)
    
    dispatcher.add_handler(CommandHandler("start", start_command))
    dispatcher.add_handler(CommandHandler("freeproxyip", freeproxyip_command))
    dispatcher.add_handler(CommandHandler("cancel", cancel_conversation))
